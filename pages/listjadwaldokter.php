<?php 
    ...
    from jadwal inner join poliklinik inner join dokter on jadwal.kd_dokter=dokter.kd_dokter and jadwal.kd_poli=poliklinik.kd_poli 
    where dokter.status='1' ".(isset($keyword)?" and (dokter.nm_dokter like '%$keyword%' or jadwal.hari_kerja like '%$keyword%' or poliklinik.nm_poli like '%$keyword%')":"")." order by jadwal.hari_kerja,jadwal.kd_dokter");
    $a=array();
    while($rsqueryjadwal = mysqli_fetch_array($queryjadwal)) {
        array_push($a,$rsqueryjadwal);
        // echo "<tr>
